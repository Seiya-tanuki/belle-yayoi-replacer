...
## Purpose
`clients/<CLIENT_ID>/config/category_overrides.json` is a per-client editable overlay for debit account defaults.
The file is full-expanded: it always contains every `category_key` from `lexicon/lexicon.json`.

## Schema
Top-level keys:
- `schema`: string, must be `belle.category_overrides.v1`
- `client_id`: string
- `generated_at`: ISO-8601 UTC timestamp
- `note_ja`: guidance text
- `overrides`: object mapping `category_key` -> `{ "debit_account": "<ACCOUNT_NAME_STRING>" }`
...
2. Load validated overrides from `clients/<CLIENT_ID>/config/category_overrides.json`.
