...
class LocalUiClientBootstrapTests(unittest.TestCase):
    ...
    def test_preview_output_shape_contains_only_minimal_display_fields(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                {"line_ids", "category_key", "category_label", "replacement_account"},
                set(payload["sections"][0]["rows"][0].keys()),
            )
            self.assertEqual(("receipt", "credit_card_statement"), tuple(payload["sections"][0]["rows"][0]["line_ids"]))
    ...
    def test_preview_prefers_label_ja_for_display_when_available(self) -> None:
        ...
        try:
            ...
            self.assertEqual(("receipt", "credit_card_statement"), state.preview.sections[0].rows[0].line_ids)
