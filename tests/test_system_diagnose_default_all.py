...
def _build_default_all_fixture(
    temp_root: Path,
    real_repo_root: Path,
    *,
    card_config_obj: dict | None = None,
) -> Path:
    script_source = (
        real_repo_root
        / ".agents"
    ...
    if card_config_obj is None:
        card_config_obj = json.loads(_minimal_credit_card_line_config_json())
    _write_text(
        card_template_root / "config" / "credit_card_line_config.json",
        json.dumps(card_config_obj, ensure_ascii=False),
    )
...
def _run_default_all_diagnose(temp_root: Path, script_target: Path) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
...
class SystemDiagnoseDefaultAllTests(unittest.TestCase):
    def test_default_all_is_bootstrap_safe_and_reports_each_line(self) -> None:
        ...
        try:
            script_target = _build_default_all_fixture(temp_root, real_repo_root)
            proc = _run_default_all_diagnose(temp_root, script_target)
    ...
    def test_default_all_fails_when_credit_card_target_payable_placeholder_names_missing(self) -> None:
        real_repo_root = Path(__file__).resolve().parents[1]
        if shutil.which("git") is None:
            self.skipTest("git executable is required for self-contained diagnose fixture setup")
        temp_root = real_repo_root / ".tmp" / f"diagnose_default_all_cc_missing_payable_{uuid4().hex}"
        temp_root.mkdir(parents=True, exist_ok=False)
        try:
            card_config_obj = json.loads(_minimal_credit_card_line_config_json())
            card_config_obj.pop("target_payable_placeholder_names", None)
            script_target = _build_default_all_fixture(
                temp_root,
                real_repo_root,
                card_config_obj=card_config_obj,
            )
            proc = _run_default_all_diagnose(temp_root, script_target)
            combined = f"{proc.stdout}\n{proc.stderr}"

            self.assertNotEqual(0, proc.returncode, msg=combined)
            self.assertIn("- credit_card_statement: NO-GO", combined)
            latest_path = temp_root / "exports" / "system_diagnose" / "LATEST.txt"
            self.assertTrue(latest_path.exists(), msg=combined)
            report_name = latest_path.read_text(encoding="utf-8").strip()
            report_text = (temp_root / "exports" / "system_diagnose" / report_name).read_text(encoding="utf-8")
    ...
    def test_default_all_fails_when_credit_card_canonical_payable_thresholds_invalid(self) -> None:
        real_repo_root = Path(__file__).resolve().parents[1]
        if shutil.which("git") is None:
            self.skipTest("git executable is required for self-contained diagnose fixture setup")
        temp_root = real_repo_root / ".tmp" / f"diagnose_default_all_cc_invalid_canonical_{uuid4().hex}"
        temp_root.mkdir(parents=True, exist_ok=False)
        try:
            card_config_obj = json.loads(_minimal_credit_card_line_config_json())
            card_config_obj["teacher_extraction"]["canonical_payable_thresholds"]["min_p_majority"] = 0
            script_target = _build_default_all_fixture(
                temp_root,
                real_repo_root,
                card_config_obj=card_config_obj,
