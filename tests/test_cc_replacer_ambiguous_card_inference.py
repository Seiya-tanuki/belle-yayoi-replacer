...
def _write_cc_config(line_root: Path) -> None:
    cfg = {
        ...
        "thresholds": {
            ...
            "file_level_card_inference": {"min_votes": 1, "min_p_majority": 0.6},
        },
        "teacher_extraction": {
...
class CCReplacerAmbiguousCardInferenceTests(unittest.TestCase):
    def test_ambiguous_file_level_card_inference_keeps_payable_sub_empty(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            run_dir = line_root / "outputs" / "runs" / "R_TEST_CC_AMBIG"
            ...
            manifest = replace_credit_card_yayoi_csv(
                ...
                config=config,
                run_dir=run_dir,
                artifact_prefix="target_01_R_TEST_CC_AMBIG",
            )

            file_inference = manifest.get("file_card_inference") or {}
            self.assertEqual("FAIL", file_inference.get("status"))
            self.assertGreater(int(file_inference.get("votes_total") or 0), 0)
