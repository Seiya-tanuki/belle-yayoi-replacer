...
class CCPartialMatchSuccessTests(unittest.TestCase):
    def test_partial_match_cainz_to_cainz_home_updates_account_and_card_votes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            run_dir = line_root / "outputs" / "runs" / "R_TEST_CC_PARTIAL_SUCCESS"
            ...
            manifest = replace_credit_card_yayoi_csv(
                ...
                config=config,
                run_dir=run_dir,
                artifact_prefix="target_01_R_TEST_CC_PARTIAL_SUCCESS",
            )

            rows = _read_rows(out_path)
            self.assertEqual([ACCOUNT_SUPPLIES, ACCOUNT_SUPPLIES, ACCOUNT_SUPPLIES], [r[COL_DEBIT_ACCOUNT] for r in rows])
            self.assertEqual(["CARD_A", "CARD_A", "CARD_A"], [r[COL_CREDIT_SUBACCOUNT] for r in rows])

            file_inference = manifest.get("file_card_inference") or {}
            self.assertEqual("OK", file_inference.get("status"))
            self.assertGreaterEqual(int(file_inference.get("votes_total") or 0), 3)
            self.assertEqual("CARD_A", file_inference.get("inferred_payable_subaccount"))

            partial = manifest.get("partial_match") or {}
            self.assertGreaterEqual(int(partial.get("account_partial_rows_used") or 0), 1)
            self.assertGreaterEqual(int(partial.get("votes_partial_used") or 0), 1)
